...
class Polygon:
    ...
    def __init__(self, root, canvas, pts, radius=4):
        ...
        self.polygon = self.canvas.create_polygon(self.flatten(), outline="red", fill="", tag="Quad")
        self.draw_points()
    ...
    def show_type(self):
        ...
        self.type_text = self.canvas.create_text(
            *center, anchor=tk.CENTER, font=("Times", FONT_SIZE, "bold"), text=text
        )
    ...
    def motion(self, event):
        ...
        self.update_point(pt, self.point_in_use.canvasx(event.x), self.point_in_use.canvasy(event.y))
        self.update_polygon()
        self.draw_points()
